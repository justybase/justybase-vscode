...
- **Inline SQL Variables**: Declare execution-scoped values directly in SQL with SAS-style `%let` directives, for example `%let points_cutoff = 20;`, then reuse them as `$points_cutoff`, `${points_cutoff}`, or `&points_cutoff`. Text values can be declared as SQL literals, for example `%let region = 'EAST';`, so substitution keeps the quotes. `%EVAL(...)` supports simple arithmetic, `%SQL(...)` substitutes the first value from an inner query, `%SQLLIST(...)` substitutes a SQL literal list from an inner query, `%PUT ...;` writes resolved messages to the execution log with a `>>> %PUT:` prefix, and all directives are stripped before execution.
- **Examples**: Inline SQL variable declarations support `%let`, `%EVAL(...)`, `%SQL(...)`, `%SQLLIST(...)`, and `%PUT ...;` for reusable query workflows and log-friendly debugging.
