...
describe('handleHydrate executingSources dedup', () => {
    ...
    it('restores and synchronizes the missing Logs slot before the first data chunk', () => {
        const win = window as any;
        ...
        const synchronizedRows = [['10:00', '--- New Execution Started ---']];
        handleAppendRows({
            command: 'appendRows',
            sourceUri,
            resultSetIndex: 0,
            ...
            fromRow: 0,
            totalRows: 1,
            logExecutionTimestamp: 20,
            isLog: true,
            isLastChunk: false,
            limitReached: false,
        });

        expect(win.resultSets[0].data).toEqual(synchronizedRows);
        expect(win.resultSets[0].executionTimestamp).toBe(20);
        expect(win.resultSets[1].data).toEqual([[42]]);
    });

    it('restores the missing Logs slot when a log delta arrives first', () => {
        const win = window as any;
        const protocol = require('../../media/resultPanel/protocol.js') as { postHostMessage: jest.Mock };
        const { handleAppendRows } = require('../../media/resultPanel/messages.js') as {
            handleAppendRows: (message: Record<string, unknown>) => void;
