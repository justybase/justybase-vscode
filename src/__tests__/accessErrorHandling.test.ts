...
describe('Access error handling', () => {
    ...
    it('recognizes UCanAccess sort-order 1045 errors, including wrapped messages', () => {
        expect(isAccessUnsupportedSortOrderError(
            new Error('net.ucanaccess.jdbc.UCAExc:::5.0.1 SortOrder[1045] is not supported'),
        )).toBe(true);
        expect(isAccessUnsupportedSortOrderError('unsupported collating sort order')).toBe(true);
        expect(isAccessUnsupportedSortOrderError('UCAExc: unrelated syntax error')).toBe(false);
    });

    it('recognizes sort-order failures in an Error.cause chain', () => {
        const outer = new Error('Import failed');
        outer.cause = new Error('UCAExc::: unsupported collating sort order SortOrder[1045,0]');
    ...
    it('formats an Access missing-table error with the table from the old SQL', () => {
        const error = new Error('UCAExc::: user lacks privilege or object not found');
    ...
    it('logs the raw error while showing the friendly modal', async () => {
        ...
        await expect(presentAccessError(
            new Error('UCAExc:::5.0.1 SortOrder[1045,0] is not supported'),
            { databaseKind: 'access', outputChannel, operation: 'Import' },
        )).resolves.toBe(true);

        expect(outputChannel.appendLine).toHaveBeenCalledWith(
            '[Import] Raw Access error: UCAExc:::5.0.1 SortOrder[1045,0] is not supported',
        );
    });
