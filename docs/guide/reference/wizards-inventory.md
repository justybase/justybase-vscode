...
- **SQLite tools** — add/drop index, view indexes/FKs, and maintenance (vacuum,
...
3. **Foreign key and check constraint wizards.** Provider-backed MySQL,
   PostgreSQL, Oracle, MSSQL, and desktop/companion creation flows remain
   missing.
...
8. **Remaining dialect-signature designers.** Full MergeTree table definitions
   (ORDER BY/PARTITION BY/TTL), Oracle/SQL Server storage-specific designers,
   and provider-backed edit plans remain.
