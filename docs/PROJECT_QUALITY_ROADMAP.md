...
At the baseline, the dependency check protected seven configured layers. The
current desktop-only graph retains five layers: contracts, shared packages,
desktop extension, webviews, and companions. It resolves workspace and alias
imports, rejects forbidden platform dependencies, and fingerprints strongly
connected components. A new forbidden edge or cycle fails the gate.
