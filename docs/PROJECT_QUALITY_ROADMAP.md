...
| CQ04 | P0 | L | Webview protocol owner | planned | Replace Result Panel `Record<string, unknown>` and catch-all command messages with exhaustive discriminated unions. Validate messages at untrusted boundaries and add compile-time sync plus negative runtime tests. |
| CQ05 | P0 | M | Result state owner | planned | Wrap persisted grid state in a versioned envelope. Support legacy timestamp keys, stable `resultSetId` keys, migrations for supported versions, and safe reset for corrupt or future state. |
