...
| CQ08 | P1 | M | Web/Electron architecture owners | done | After Electron became a second client consumer, centralized typed HTTP, CSRF, downloads, WebSocket event validation, sequence de-duplication, and bounded reconnects in `@justybase/api-client`. Web retains only its React provider; Electron retains only same-origin transport policy and compatibility exports. Added package boundary/build wiring and isolated transport tests while preserving both product test suites. Evidence: `packages/api-client/`, `apps/web/src/api.ts`, `apps/electron/src/renderer/api.ts`, `quality/architecture-rules.json`, `docs/ARCHITECTURE.md`, `npm run test:api-client`, `npm run test:web`, `npm run test:electron`, `npm run check:architecture`, `npm run build:web`, and `npm run build:electron`. Verified 2026-09-13. |
...
| TQ03 | P0 | XL | UI owners | in-progress | Result Panel now has an executable host/state contract and deep scroll/browser/Extension Host evidence covering stable identity, pinned/index transitions, source removal, streaming cancellation, late-chunk rejection, active-source recovery, filtering, sorting, grouping, aggregate analysis, hidden-view recovery, stream ordering, and both Web/Electron shared-renderer paths. Current Linux gates include Extension Host result/filter-performance, Electron live 11/11 checks, Playwright performance 6/6, and rendering 19/19; Windows/Remote-WSL and remaining stateful panels remain open. Evidence: `src/__tests__/resultPanelStateContract.test.ts`, `src/__tests__/resultPanelView.scroll.test.ts`, `docs/RESULT_PANEL_REGRESSION.md`, `scripts/extensionHost/extensionHost.js`, `test-harness/tests/table-rendering.spec.ts`, `test-harness/tests/data-grid-performance.spec.ts`, `scripts/electron/electronLiveSmoke.js`. Verified 2026-09-12. Extend the same contract to the remaining stateful panels. |
| TQ04 | P0 | L | Web owner | in-progress | React coverage now includes shared workspace controls, connection/dialect guards, Result Grid state/actions, schema actions, import/export paths, and Electron connection/schema surfaces. Continue adding focused Testing Library coverage for tabs, preferences, cancellation, and reload restoration; the shipped surface is not yet fully covered. Evidence: `apps/web/src/workspacePanels.test.tsx`, `apps/web/src/sharedUiAdapter.edge.test.tsx`, `apps/electron/tests/connectionPanel.test.tsx`, `apps/electron/tests/schemaExplorer.test.tsx`. Verified 2026-09-12. |
| TQ05 | P0 | L | Browser/host test owner | in-progress | High-traffic authoring and Result Grid paths now run through real browser, Electron, and Extension Host gates with observable readiness, sanitized reports, and live viewport metrics. Extend the same boundary coverage to the remaining panels and CI environments. Evidence: `test-harness/tests/web-sql-workspace.spec.ts`, `test-harness/tests/data-grid-performance.spec.ts`, `scripts/extensionHost/extensionHostAuthoringSmoke.js`, `scripts/electron/electronLiveSmoke.js`. Verified 2026-09-12. |
...
| TQ08 | P1 | XL | Data movement owner | in-progress | Import/export now has round-trip and failure coverage for nulls, Unicode, large integers, decimals, timestamps/time zones, duplicate headers, empty/header-only files, compressed CSV, cancellation, partial failure, bounded rows, and temporary-resource cleanup. Migration round trips and the remaining spreadsheet edge matrix are still open. Evidence: `apps/api/tests/tabularImportRuntime.test.ts`, `apps/api/tests/server.test.ts`, `apps/electron/tests/dataMovement.test.ts`, `apps/web/src/ImportPanel.test.tsx`. Verified 2026-09-12. |
| TQ09 | P1 | XL | Dialect owners | in-progress | A canonical database-kind catalog now drives Web/Electron connection forms, and selected authoring profiles drive completion, snippets, diagnostics, signatures, formatting, semantic tokens, and guarded capability states. The API runtime registry remains limited to Netezza/SQLite/DuckDB; finish the reusable metadata/DDL/import contract and add controlled remote runtime gates before promotion. Evidence: `packages/contracts/src/database/index.ts`, `apps/api/src/sqlAuthoring.ts`, `apps/api/src/databaseRuntime/registry.ts`, `test-harness/tests/web-sql-workspace.spec.ts`, `scripts/electron/electronLiveSmoke.js`. Verified 2026-09-12. |
