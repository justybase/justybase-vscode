...
Status: Shared Web is the production/default renderer and is verified locally
with the controlled Chromium/API workflow. The cross-platform matrix is
committed for Linux, Windows, and macOS; platform-specific evidence is tracked
by CI. VS Code keeps its host-owned renderer and Electron consumes the shared
React renderer.

This is the operational inventory for the shared Web/Electron renderer and its
VS Code adapter boundary. Historical R9/R10 extraction decisions remain in
[the refactoring plan](REFACTORING_PLAN.md), but the retired Dockyard rollout
is no longer the Web runtime contract. It records the ownership and behavior
that a vertical slice must preserve across the Web editor/API, Electron, and
VS Code.
...
always mounts `SharedWebWorkspace`; `VITE_UI_MODE` and the Dockyard runtime
rollback path are removed. VS Code keeps its existing host path, and Electron
uses the shared React surface.

Shared Web persistence is a versioned, user-scoped envelope containing only
document metadata/content, document order, active document, and connection
context. Legacy `tabs` and older Web keys migrate into that envelope; SQL
credentials, result rows, DOM/layout objects, and runtime handles are excluded.
...
- Shared Web cross-platform gate: `.github/workflows/web-shared.yml` runs the
  real `npm run test:playwright:web-shared` Chromium/API scenario on Linux,
...
| --- | --- | --- | --- | --- | --- | --- |
| Shell and workspace tabs | `SharedWebWorkspace` with durable document persistence and stable source identities | `apps/electron/src/renderer/App.tsx` uses shared store/presentation | VS Code workbench and webviews | `@justybase/ui-core` + `@justybase/ui-react`; host layout remains adapter-owned | Shared Web default; Electron shared shell; VS Code adapter-backed | Shared workspace owner; retain platform-specific shell differences only behind explicit capability boundaries. |
| SQL editor and LSP | Monaco in `SharedWebWorkspace`, `@justybase/ui-monaco`, API `sql-core` adapter | Shared Monaco editor shell through `@justybase/ui-monaco`, dialect profile, completion and execution adapter | VS Code editor/LSP providers | Shared document ports + `sql-core` semantics; browser Monaco boundary is `ui-monaco` | Shared Web/Electron authoring; VS Code path remains separate | Editor/LSP owners; dialect and host-specific authoring differences stay adapter-owned. |
...
| Query execution and cancellation | `SharedWebWorkspace` maps API/WebSocket events through the shared execution controller | Renderer execution adapter maps the authenticated API/WebSocket stream to `ui-core`; the development/test shell is shared-only | `StreamingManager` and desktop execution adapter | Shared execution state ports; runtime remains backend-owned | Shared Web/Electron renderer; VS Code adapter-backed | Execution owners; ordered events, cancellation, reconnect, and cleanup remain required gates. |
| Result panel and Data Grid | `SharedWebWorkspace` uses the shared result panel/Data Grid with per-document result identity | Shared editor, result tabs, grid, filtering, copy/export, grouping drop zone, and scroll state are wired through the renderer adapter | `media/resultPanel/sharedView.tsx` opt-in with legacy fallback | `@justybase/ui-core` + `@justybase/ui-react` result ports and canonical token stylesheet | Shared Web/Electron renderer; VS Code adapter-backed | Result Panel owner; retain host-specific virtualization only where capability requires it. |
| Schema navigation and metadata | Shared adapter maps API schema tree, database context and inspector | Authenticated schema tree, search, columns, DDL, top rows, import and designer entry points | metadata cache and schema tree | Shared explorer state + `metadata-core` | Shared Web/Electron adapter-backed; VS Code remains host-owned | Metadata owner; preserve restart/invalidation and SchemaProvider gates. |
| Query history | Shared history view backed by API `/api/history` | Authenticated profile-scoped history view with open, copy, refresh and rerun actions | `QueryHistoryManager` uses `context.globalStorageUri`; `globalState` is legacy migration/fallback | Shared history view state; repository scope remains adapter-owned | Shared Web presentation; Electron adapter slice is wired | History owner; scope/retention remains product-owned. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Foundation (`ui-core`, `ui-react`, adapters) | Shared Web default; Electron shared-only development/test shell | Implemented production Shared Web composition | Implemented development/test shell with authenticated main-owned session; shared-only renderer | Shared Result Panel adapter is opt-in; legacy fallback retained | Architecture, type, lint, lifecycle, contract, coverage, and Electron live-window gates | Shared owner; keep the boundary secret-free and additive. |
| Results | Shared Web/Electron renderer; VS Code host renderer remains separate | Shared Result Panel/Data Grid composition is the default | Authenticated shared editor/query/result composition is the only renderer path in the development/test shell; no legacy fallback | Shared Result Panel adapter is opt-in; legacy fallback retained | Result identity/scroll tests, Web/media/Electron coverage, browser and Extension Host evidence, and passed Electron live-window smoke | Result Panel owner; preserve explicit platform capability differences. |
| Workspace/authoring | Shared Web default; Electron shared-only shell | Shared shell/document/execution composition is the production path | Shared editor shell is the only renderer path; API/LSP and execution wiring remain open | Existing editor/LSP semantics and host ownership remain | LSP/completion/parity, authoring Extension Host, reconnect, and persistence tests | SQL/editor owners; preserve public commands. |
...
| Retired Web Dockyard workspace | Removed from Web runtime; no rollback flag | Not mounted | VS Code host/workbench remains product-owned | n/a | Shared Web browser gate and source-level absence of Web mode branching | Keep archived vendor/source files only where required for historical/vendor records; do not reintroduce as a runtime renderer. |
