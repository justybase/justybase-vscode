...
import type * as NetezzaSqlLexerModule from '../netezza/lexer';
import { registerQueryClauseComparisonRules } from './queryClauseComparisonRules';
...
function shouldLogParserPerformance(): boolean {
  return getRuntimeEnvironment()?.JUSTYBASE_PARSER_PERF === "1";
}
...
export class BaseSqlParser extends CstParser {
  ...
  constructor(protected readonly tokenBundle: SqlParserTokenBundle) {
    ...
    if (runtimeEnvironment?.SQLPARSER_SKIP_VALIDATIONS !== undefined) {
      const v = String(runtimeEnvironment.SQLPARSER_SKIP_VALIDATIONS).toLowerCase();
