...
import { getOrderedReferenceTokens, isCstNode, isToken } from "./referenceTokenCollector"
...
export function collectSqlSymbolUsages(sql: string): SqlSymbolUsage[] {
...
export function resolveSqlRenameSymbol(
    ...
    const resolvedParseResult = parseResult ?? parseNetezzaSqlStatements({ sql })
    if (
