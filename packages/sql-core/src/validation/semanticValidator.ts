import { Semicolon } from "../netezza/lexer";
import {
  NETEZZA_SQL_PARSING_RUNTIME,
  parseNetezzaSqlStatements,
  type NetezzaSqlParseResult,
} from "../parser/runtime";
import { SqlVisitor } from "./visitor/sqlVisitorCore";
...
export type { ScopeSeed, StatementBoundary } from "./types";
...
export type NetezzaSqlSemanticValidationResult = ValidationResult;
...
export class NetezzaSqlSemanticValidator {
  ...
  constructor(
    ...
    validationProfile: DatabaseSqlValidationProfile = NETEZZA_SQL_VALIDATION_PROFILE,
  ) {
    if (validationProfile.databaseKind && validationProfile.databaseKind !== "netezza") {
      throw new TypeError(`NetezzaSqlSemanticValidator cannot use ${validationProfile.databaseKind} validation profile.`);
    }
  ...
  validate(sql: string): NetezzaSqlSemanticValidationResult {
    // Treat semicolon-only scripts as valid no-op statements.
    if (/^\s*;+\s*$/.test(sql)) {
      this.visitor = new SqlVisitor(this.schemaProvider, this.validationProfile);
      return {
        valid: true,
        errors: [],
        warnings: [],
  ...
  validateFromParseResult(
    ...
    if (parseResult.runtime !== NETEZZA_SQL_PARSING_RUNTIME) {
      throw new TypeError("NetezzaSqlSemanticValidator requires a Netezza parse result.");
    }
  ...
  validateStatementText(
    ...
    const statementValidator = new NetezzaSqlSemanticValidator(
      this.schemaProvider,
  ...
  runPreParseChecks(
    lexResult: NetezzaSqlParseResult["lexResult"],
  ): PreParseCheckResult {
    const errors: ValidationError[] = [];
  ...
  private resultFromDiagnostics(diagnostics: ValidationError[]): NetezzaSqlSemanticValidationResult {
  ...
  private remapStatementIssue(
    ...
    return {
      ...issue,
      message: this.remapStatementMessage(issue.message, statementOffset),
  ...
  private remapStatementMessage(message: string, statementOffset: number): string {
    if (statementOffset === 0) return message;
    return message.replace(/(\boffset\s*:\s*)(\d+)/gu, (_match, prefix: string, localOffset: string) =>
      `${prefix}${statementOffset + Number(localOffset)}`,
    );
  }
...
export const netezzaSqlSemanticValidator = new NetezzaSqlSemanticValidator();
