...
import type { DatabaseSqlFormatterProfile } from "@justybase/contracts";

export type SqlKeywordCase = "upper" | "lower" | "preserve";

export interface NetezzaFormatOptions {
  tabWidth?: number;
...
export function formatNetezzaSql(
  sql: string,
  options: NetezzaFormatOptions = {},
): string {
  return formatSqlWithProfile(sql, NETEZZA_SQL_AUTHORING.formatter, options);
}

/**
 * Formats SQL with a dialect formatter profile.  Keeping the formatter
 * profile separate from the implementation lets platform-neutral clients
 * select the requested database dialect without importing the desktop
 * registry (which depends on VS Code and companion extensions).
 */
export function formatSqlWithProfile(
  sql: string,
  formatterProfile: DatabaseSqlFormatterProfile,
  options: NetezzaFormatOptions = {},
): string {
  if (!sql.trim()) {
    return sql;
  ...
  return alignClauseListColumns(formatted);
}
...
function alignClauseListColumns(sql: string): string {
