...
import { isDesignerCapabilityOperationSupported } from './designer';
import { formatTableDesignerIdentifier } from './identifier';
...
export function isTableDesignerSupported(kind: string | undefined, context?: TableDesignerRuntimeContext): boolean {
    ...
    return profile.supported && isDesignerCapabilityOperationSupported(capability, 'create');
}
...
export function getTableDesignerUnsupportedReason(kind: string | undefined, context?: TableDesignerRuntimeContext): string | undefined {
    const profile = getTableDesignerProfile(kind);
    const capability = getTableDesignerCapability(kind, context);
    if (!isDesignerCapabilityOperationSupported(capability, 'create')) {
...
function quoteIdentifier(identifier: string, kind: string): string {
    return formatTableDesignerIdentifier(identifier, kind);
}
