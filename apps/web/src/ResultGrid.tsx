...
import { CellValueViewer, DataGrid, downloadBlobFile, DataGridColumnFilterPanel, DataGridColumnMenu, createDataGridClipboardPayload, formatDataGridCellValue, formatDataGridClipboard, inferDataGridColumnMetadata, isDataGridNumericColumn, isDataGridTemporalColumn, processDataGridRows } from '@justybase/ui-react';
...
import { aggregateResultRows, collectFullSpoolRows, type ResultColumn } from '@justybase/result-core';
...
/**
 * The API still transfers a bounded page at a time, but that is an
 * implementation detail of the infinite result surface. The user sees one
 * continuous, virtualized grid and never has to navigate pages.
 */
const RESULT_ROW_BATCH_SIZE = 10_000;
...
export function ResultGrid({ queryId, statementIndex = 0, result, onEditRow }: { queryId: string; statementIndex?: number; result: ResultState; onEditRow?(values: unknown[]): void }): ReactElement {
  ...
  const [scrollRowHeight, setScrollRowHeight] = useState<number | undefined>(undefined);
  ...
  const totalRowsRef = useRef(result.totalRows);
  const hasMoreRowsRef = useRef(false);
  const pageHydrationRef = useRef(new Set<string>());
  const serverLoadSignatureRef = useRef<string | undefined>(undefined);
  ...
  const loadResultPage = useCallback(async (generation: number, offset: number, replace: boolean): Promise<void> => {
    if (!queryId || !result.sessionId) return;
    const requestKey = `${generation}:${offset}`;
    if (pageHydrationRef.current.has(requestKey)) return;
    pageHydrationRef.current.add(requestKey);
    setLoading(true);
    try {
      const response = await api.queryPage(queryId, {
        statementIndex,
        offset,
        limit: RESULT_ROW_BATCH_SIZE,
        sorting: requestSorting,
        columnFilters: requestFilters,
        globalFilter,
      });
      if (generation !== requestGeneration.current) return;
      const pageOffset = Number.isFinite(response.offset) ? Math.max(0, response.offset) : offset;
      const responseHasMore = response.hasMore ?? pageOffset + response.rows.length < response.totalRows;
      const nextRows = replace || pageOffset === 0
        ? response.rows.map(row => [...row])
        : [
          ...loadedRowsRef.current.slice(0, pageOffset),
          ...response.rows.map(row => [...row]),
          ...loadedRowsRef.current.slice(pageOffset + response.rows.length),
        ];
      loadedRowsRef.current = nextRows;
      totalRowsRef.current = response.totalRows;
      hasMoreRowsRef.current = responseHasMore;
      setRows(nextRows);
      setTotalRows(response.totalRows);
      setHasMoreRows(responseHasMore);
    } catch (reason: unknown) {
      if (generation === requestGeneration.current) setError(reason instanceof Error ? reason.message : 'Could not load result rows.');
    } finally {
      pageHydrationRef.current.delete(requestKey);
      if (generation === requestGeneration.current && ![...pageHydrationRef.current].some(key => key.startsWith(`${generation}:`))) setLoading(false);
    }
  }, [api, globalFilter, queryId, requestFilters, requestSorting, result.sessionId, statementIndex]);

  useEffect(() => {
    requestGeneration.current += 1;
    pageHydrationRef.current.clear();
    loadedRowsRef.current = result.rows;
    totalRowsRef.current = result.totalRows;
    ...
    setScrollRowHeight(Number.isInteger(saved?.scrollRowHeight) && (saved?.scrollRowHeight ?? 0) >= 0 ? saved?.scrollRowHeight : undefined);
  ...
  useEffect(() => {
    ...
    try {
      const state: SavedGridState = { sorting, columnFilters, columnFilterDefinitions, globalFilter, columnVisibility, columnPinning, columnOrder, columnWidths, grouping: gridGrouping, scrollTop, scrollLeft, scrollAnchorRow, ...(scrollRowHeight === undefined ? {} : { scrollRowHeight }) };
  ...
  }, [gridKey, gridHydratedKey, resultSetId, sorting, columnFilters, columnFilterDefinitions, globalFilter, columnVisibility, columnPinning, columnOrder, columnWidths, gridGrouping, scrollTop, scrollLeft, scrollAnchorRow, scrollRowHeight, storage]);

  useEffect(() => {
    if (!queryId || !result.sessionId) return;
    if (gridHydratedKey !== gridKey) return;
    const loadSignature = JSON.stringify({ queryId, resultSetId, sessionId: result.sessionId, statementIndex, status: result.status, globalFilter, columnFilters: requestFilters, sorting: requestSorting });
    if (serverLoadSignatureRef.current === loadSignature) return;
    serverLoadSignatureRef.current = loadSignature;
    ...
    setTotalRows(result.totalRows);
    setHasMoreRows(true);
    void loadResultPage(generation, 0, true);
  }, [globalFilter, gridHydratedKey, gridKey, loadResultPage, queryId, requestFilters, requestSorting, result.sessionId, result.status, resultSetId, statementIndex]);

  const loadMoreRows = useCallback(async (): Promise<void> => {
    if (!result.sessionId || !hasMoreRowsRef.current) return;
    const generation = requestGeneration.current;
    const offset = loadedRowsRef.current.length;
    if (offset >= totalRowsRef.current) return;
    await loadResultPage(generation, offset, false);
  }, [loadResultPage, result.sessionId]);
  ...
  async function openColumnFilter(request: DataGridColumnFilterRequest): Promise<void> {
    ...
    const anchorGap = 5;
    const popupWidth = Math.min(340, Math.max(240, window.innerWidth - viewportMargin * 2));
    const popupHeight = Math.min(520, Math.max(160, window.innerHeight - viewportMargin * 2));
    const minimumUsableHeight = Math.min(280, popupHeight);
    const left = Math.min(
      Math.max(viewportMargin, anchor.left),
      Math.max(viewportMargin, window.innerWidth - popupWidth - viewportMargin),
    );
    const availableHeightBelow = Math.max(0, window.innerHeight - anchor.bottom - viewportMargin - anchorGap);
    const availableHeightAbove = Math.max(0, anchor.top - viewportMargin - anchorGap);
    const placeAbove = availableHeightBelow < minimumUsableHeight && availableHeightAbove > availableHeightBelow;
    const preferredTop = placeAbove ? anchor.top - popupHeight - anchorGap : anchor.bottom + anchorGap;
    const top = Math.max(viewportMargin, Math.min(preferredTop, window.innerHeight - popupHeight - viewportMargin));
    setFilterMenu({
    ...
    try {
      ...
      const selectedKeys = definition?.operator === 'in'
        ? definition.values.map(filterValueKey).filter(key => options.some(option => option.key === key))
  ...
  function clearColumnFilter(): void {
    ...
    if (!menu) return;
    const id = String(menu.columnIndex);
    setColumnFilterDefinitions(previous => {
      const next = { ...previous };
      delete next[id];
      return next;
    });
    setColumnFilters(previous => previous.filter(item => item.id !== id));
    closeColumnFilter();
  }
  ...
  async function pivotResults(config: PivotConfig): Promise<void> {
    ...
    try {
      const groupedRows = result.sessionId
        ? await (async () => {
          const response = await api.group(queryId, { statementIndex, groupByColumnIndices: [config.rowColumnIndex, config.pivotColumnIndex], aggregates: [{ function: 'sum', columnIndex: config.valueColumnIndex }], globalFilter, columnFilters: requestFilters, groupLimit: MAX_QUERY_GROUP_LIMIT });
          if (response.totalGroups > response.rows.length) {
            setNotice(`Pivot truncated to the first ${response.rows.length.toLocaleString()} of ${response.totalGroups.toLocaleString()} groups. Refine filters for a complete pivot.`);
          } else {
            setNotice(previous => previous.startsWith('Pivot truncated to the first') ? '' : previous);
          }
  ...
  const displayRows = useMemo(() => {
    ...
    const simpleFilters = Object.fromEntries(columnFilters
      ...
      .map(item => {
        const numericIndex = Number(item.id);
        const key = Number.isInteger(numericIndex) && numericIndex >= 0 && numericIndex < result.columns.length
          ? result.columns[numericIndex]!
          : item.id;
        return [key, typeof item.value === 'string' ? item.value : ''];
      }));
  ...
  const sharedGridView = useMemo(() => ({
    globalFilter,
    columnFilters: Object.fromEntries(columnFilters.map(item => {
      const numericIndex = Number(item.id);
      const key = Number.isInteger(numericIndex) && numericIndex >= 0 && numericIndex < result.columns.length
        ? result.columns[numericIndex]!
        : item.id;
    ...
    columnFilterDefinitions: Object.fromEntries(Object.entries(columnFilterDefinitions).map(([id, definition]) => {
      const numericIndex = Number(id);
      const key = Number.isInteger(numericIndex) && numericIndex >= 0 && numericIndex < result.columns.length
        ? result.columns[numericIndex]!
        : id;
    ...
    columnWidths,
  }), [columnFilterDefinitions, columnFilters, globalFilter, gridGrouping, columnOrder, columnPinning.left, columnWidths, result.columns, sorting]);
  ...
  function updateSharedGridView(patch: Partial<UiResultViewState>): void {
    if (patch.globalFilter !== undefined) setGlobalFilter(patch.globalFilter);
    ...
    if (patch.scrollRowHeight !== undefined) setScrollRowHeight(Math.max(0, patch.scrollRowHeight));
  }

  function handleGridScroll(position: GridScrollPosition): void {
    if (position.resultSetId !== resultSetId
      || (position.top === scrollTop
        && position.left === scrollLeft
        && position.anchorRow === scrollAnchorRow
        && position.rowHeight === scrollRowHeight)) return;
    updateSharedGridView({ scrollTop: position.top, scrollLeft: position.left, anchorRow: position.anchorRow, scrollRowHeight: position.rowHeight });
  }
  ...
  function exportSelection(payload: DataGridCopyPayload, format: 'csv' | 'json'): void {
    setError('');
    try {
      ...
      setError('Could not export the selection.');
    }
  }

  function cancelCopyAll(): void {
  ...
  async function copyFullSpool(): Promise<void> {
    if (copyingAll) {
      cancelCopyAll();
      return;
    }
    ...
    try {
      const allRows = await collectFullSpoolRows(
        ...
        },
        {
          ...
          onProgress: progress => {
            ...
            setCopyProgress(`${progress.loadedRows.toLocaleString()} / ${progress.totalRows.toLocaleString()} rows…`);
          },
        },
      );
      if (controller.signal.aborted) return;
      await copyGridPayload({ columns: gridColumns, rows: allRows });
      ...
      setError(reason instanceof Error ? reason.message : 'Could not copy the full result.');
    } finally {
  ...
  return <section className="advanced-grid">
    <div className="grid-toolbar">
      ...
      <div className="grid-tool-group grid-copy-actions"><button className="secondary small copy-btn" aria-label="Copy selected result rows" onClick={copySelection}>Copy</button><button className="secondary small copy-all-btn" aria-label="Copy full result (all rows)" title={result.sessionId ? 'Copy every filtered row from the server spool, not just the loaded page' : 'Copy all loaded rows'} onClick={() => void copyFullSpool()}>{copyingAll ? 'Copying…' : 'Copy all'}</button>{copyingAll && <button className="secondary small" aria-label="Cancel full result copy" onClick={cancelCopyAll}>Cancel</button>}{copyingAll && copyProgress && <span className="running" role="status">{copyProgress}</span>}</div>
      ...
      <DataGridColumnMenu columns={gridColumns} view={sharedGridView} onViewChange={updateSharedGridView} />
    ...
    {result.limitReached === true && <div className="grid-limit-banner" role="alert">Row limit reached — showing the first {result.totalRows.toLocaleString()} rows. Refine filters or use Export for the full spool.</div>}
    ...
    <DataGrid resultSetId={resultSetId} columns={gridColumns} rows={gridRows} totalRowCount={effectiveTotalRows} view={sharedGridView} clientProcessing={!result.sessionId} showContextMenu showColumnMenu={false} showGroupingPanel={groupPanelOpen} showInlineColumnFilters={false} onOpenColumnFilter={openColumnFilter} onViewChange={updateSharedGridView} selectedRowIndex={selectedRawIndex} scroll={{ resultSetId, top: scrollTop, left: scrollLeft, anchorRow: scrollAnchorRow, ...(scrollRowHeight === undefined ? {} : { rowHeight: scrollRowHeight }) }} onScroll={handleGridScroll} onLoadMore={result.sessionId ? loadMoreRows : undefined} onCopySelection={copyGridSelection} onExportSelection={exportSelection} onSelectionChange={selection => setRowSelection(selection ? { [String(selection.focusRow)]: true } : {})} onRowSelect={rowIndex => { const displayIndex = result.sessionId ? rowIndex : displayRows.findIndex(row => row === result.rows[rowIndex]); if (displayIndex >= 0) setRowSelection({ [String(displayIndex)]: true }); }} onViewRow={context => setDetailRowIndex(context.rowIndex)} onViewCell={openCellValue} onEditRow={onEditRow ? context => { const row = gridRows[context.rowIndex]; if (row) onEditRow([...row]); } : undefined} />
    ...
    <div className="grid-statusbar" role="status"><span>{effectiveTotalRows.toLocaleString()} rows</span><span>{loading ? `Loading… ${rows.length.toLocaleString()} loaded` : hasMoreRows ? `${rows.length.toLocaleString()} loaded · scroll for more` : 'All rows loaded'}</span></div>
  </section>;
}
