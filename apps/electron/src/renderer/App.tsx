...
export function App(): ReactElement {
  ...
  const toggleResultAnalysis = useCallback((kind: ResultAnalysisKind): void => {
    ...
    void (async (): Promise<void> => {
      try {
        if (kind === 'aggregate') {
          const response = await client.aggregate(result.executionId, {
          ...
          if (result.columns.length === 0) throw new Error('This result has no columns to group.');
          ...
          const response = await client.group(result.executionId, {
            statementIndex: result.statementIndex,
            ...options,
  ...
  }, [activeResult, closeResultAnalysis, resultAnalysis]);
  ...
  const openAnalysisCellValue = useCallback((context: DataGridCellContext): void => {
  ...
  return <CapabilityGate capability={workspaceCapability}><>
    ...
    tools={[
      { id: 'electron-explorer', title: 'Explorer', defaultDock: 'left' as const, content: <div className="electron-sidebar-content">
        ...
        <CapabilityGate capability={metadataCapability} fallback={<div className="electron-capability-muted">{metadataCapability?.reason ?? 'Schema metadata unavailable.'}</div>}><SchemaExplorer api={clientRef.current!} connectionId={selectedConnection?.id} database={database} databaseKind={runtimeDatabaseKind} refreshNonce={schemaRefreshNonce} onInsert={insertSql} onContextChange={selectSchemaContext} onObjectSelect={setSelectedObject} onOpenDesigner={openObjectDesigner} onOpenQuery={openSchemaQuery} onOpenDdl={openDdl} onImport={setImportTarget} /></CapabilityGate>
      </div> },
