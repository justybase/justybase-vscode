jest.mock('electron', () => ({
  ...
  ipcRenderer: { invoke: jest.fn(async () => ({ ok: true, auth: { status: 'authenticated' } })), on: jest.fn(), removeListener: jest.fn() },
}));
...
describe('Electron preload entrypoint', () => {
  it('exposes only the validated renderer bridge on the expected global name', async () => {
    ...
    expect(electron.contextBridge.exposeInMainWorld).toHaveBeenCalledWith('justybaseMenu', expect.any(Object));
